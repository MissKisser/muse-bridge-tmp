...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p.endswith("/responses"):
            return self.handle_responses_post()
        ...
        upstream_body, name_map = build_upstream(body)
        headers = {
        ...
        try:
            if not stream:
                with httpx.Client(timeout=TIMEOUT) as cli:
                    ...
                    comp = resp_to_chat(r.json(), model, chat_id, name_map)
                    log(f"NOSTREAM {model} 200 finish={comp['choices'][0]['finish_reason']} ({time.time()-t0:.1f}s)")
                    return self._send(200, comp)
            self.do_stream(upstream_body, headers, model, chat_id, t0, name_map)
